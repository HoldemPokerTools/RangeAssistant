...
const validateRange = (range) => {
  const ajv = new Ajv();
  if(!ajv.validate(require("./range.schema.json"), range)) throw new Error(`Invalid range: ${ajv.errorsText()}`);
}
...
const handleRange = async (fp) => {
  try {
    ...
    await dialog.showMessageBox(null, {
      ...
      detail: "The range file is not valid. Please try re-exporting the range file from the range builder",
    });
  }
}
    ...
    try {
      ...
      const {canceled, filePaths} = await dialog
        .showOpenDialog({
          ...
          filters: [
            { name: 'Range Files', extensions: ['range'] },
          ]
        });
      if (!canceled) {
...
const setMenu = (enableImport) => {
  const menu = defaultMenu(app, shell, enableImport, showOpenFileDialog, createAppWindow);
